...
class dwVerify {
	 ...
	 * ttf字体路径，数组
	 */
	public $font_family = array('fonts/arial.ttf');
	/**
	 * 简体中文开关
	 */
	public $ZH = false;
	...
	public function code() {
		...
		} else  return substr(str_shuffle('1234567890abcdefghijklmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ'), 0, $this->font_num);
	}
	/**
	 * 产生图像，指定宽高与背景颜色
	 */
	...
	public function show($code = null, $fileName = null) {
		$this->create();

        isset($code) || $code = $this->code();
		$this->verify_code = $code;
		//var_dump($code);die();
		$font_family = $this->font_family[mt_rand(0, count($this->font_family) - 1)];
		$font_family = dirname(__FILE__) . '/' . $font_family;

		for ($i = 0; $i < $this->font_num; $i++) {
			//die($font_family);
			$fontcolor = imagecolorallocate($this->img, mt_rand(0, 250), mt_rand(0, 250), mt_rand(0, 250));
			$ret = imagettftext($this->img, $this->font_size, mt_rand(-30, 30), ($this->ZH) ? ($this->font_size / 2 + $this->font_size * $i) * 1.5: $this->font_size / 2 + $this->font_size * $i, ($this->h + 
				$this->font_size) / 2, $fontcolor, $font_family, mb_substr($code, $i, 1, 'UTF-8'));
		}
		$this->makepoint();
		$this->makeline();
