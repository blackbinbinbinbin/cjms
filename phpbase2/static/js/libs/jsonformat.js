...
function formatJson(jsonObj, indent) {
     ...
     */
    function parseObj(obj, indentNum, comma, prefix) {
        var key, len = 0,
            i = 0;
        var simple = ', simple';
        for (key in obj) {
        ...
        for (key in obj) {
            if (obj.hasOwnProperty(key)) {
                i++;
                var path = prefix + ':' + key;
                objstr += spacestr + space + '<span class="json_key" data-path="' + path + '">"' +
                    key + '"</span>: ' +
                    parseValue(obj[key], indentNum + 1, i !== len, path) + '<br/>';
            }
        }
     ...
     * @return {string}           转换之后的html字符串
     */
    function parseArray(value, indentNum, comma, prefix) {
        var simple = ', simple';
        ...
        for (i = 0, len = value.length; i < len; i++) {
            arr += spacestr + space + parseValue(value[i], indentNum + 1, i !== value.length - 1, prefix) + "<br/>";
        }
     ...
     */
    function parseValue(value, indentNum, comma, prefix) {
        prefix = prefix || '';
        if (indentNum === void 0) {
            indentNum = 0;
        }
        ...
        switch (type) {
            ...
            case "Array":
                return parseArray(value, indentNum, comma, prefix + '[');
            case "Object":
                return parseObj(value, indentNum, comma, prefix + '{');
            default:
                if (value !== null) {
